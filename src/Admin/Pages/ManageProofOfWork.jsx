...
const ManageProofOfWork = () => {
    ...
    // 2. Handlers
    const resetForm = () => {
        setFormData({ title: '', category: 'screen', type: 'image', before: '', after: '', videoUrl: '', thumbnail: '', desc: '', duration: '' });
        setEditingId(null);
        setIsFormOpen(false);
    };
        ...
        <div className="min-h-screen bg-gray-50 p-6 sm:p-10">
            ...
            <div className="flex justify-between items-center mb-8">
                ...
                <div className="flex gap-3">
                    <button onClick={() => navigate('/admin/dashboard')} className="px-4 py-2 border rounded-lg hover:bg-white transition text-sm font-bold">Dashboard</button>
                    <button onClick={() => { resetForm(); setIsFormOpen(true); }} className="px-4 py-2 bg-purple-900 text-white rounded-lg hover:bg-purple-800 transition text-sm font-bold flex items-center gap-2 shadow-lg">
            ...
            {/* FORM SECTION */}
            {isFormOpen && (
                <div className="bg-white p-6 rounded-2xl shadow-xl border border-gray-200 mb-8 animate-in fade-in slide-in-from-top-4">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div className="space-y-4">
                                <div>
                                    ...
                                    <input className="w-full p-3 border rounded-xl outline-none focus:ring-2 focus:ring-purple-500" placeholder="e.g. iPhone 13 Pro Max Screen" value={formData.title} onChange={e => setFormData({...formData, title: e.target.value})} />
                                </div>
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        <label className="block text-xs font-bold text-slate-500 uppercase mb-1">Category</label>
                                        <select className="w-full p-3 border rounded-xl outline-none bg-white" value={formData.category} onChange={e => setFormData({...formData, category: e.target.value})}>
                                    ...
                                    <div>
                                        ...
                                        <select className="w-full p-3 border rounded-xl outline-none bg-white" value={formData.type} onChange={e => setFormData({...formData, type: e.target.value})}>
                                            <option value="image">Before/After Images</option>
                                ...
                                <div>
                                    ...
                                    <textarea className="w-full p-3 border rounded-xl outline-none focus:ring-2 focus:ring-purple-500" rows="3" placeholder="Brief details about the fix..." value={formData.desc} onChange={e => setFormData({...formData, desc: e.target.value})}></textarea>
                                </div>
                                <div>
                                    <label className="block text-xs font-bold text-slate-500 uppercase mb-1">Duration</label>
                                    <input className="w-full p-3 border rounded-xl outline-none focus:ring-2 focus:ring-purple-500" placeholder="e.g. 45 Mins" value={formData.duration} onChange={e => setFormData({...formData, duration: e.target.value})} />
                                </div>
                            </div>
                                ...
                                {formData.type === 'image' ? (
                                    <>
                                        <div>
                                            ...
                                            <div className="flex gap-2">
                                                <LinkIcon size={18} className="text-slate-400 mt-3"/>
                                    ...
                                    <>
                                        <div>
                                            ...
                                            <div className="flex gap-2">
                                                <Film size={18} className="text-slate-400 mt-3"/>
                                        ...
                                        <div>
                                            ...
                                            <div className="flex gap-2">
                                                <ImageIcon size={18} className="text-slate-400 mt-3"/>
                                                <input className="w-full p-3 border rounded-xl outline-none focus:ring-2 focus:ring-purple-500 text-sm" placeholder="https://..." value={formData.thumbnail} onChange={e => setFormData({...formData, thumbnail: e.target.value})} />
                                            </div>
                                        </div>
                                    </>
                                )}
                                <div className="mt-4 p-3 bg-blue-50 text-blue-700 text-xs rounded-lg">
                                    <strong>Tip:</strong> Paste direct URLs from Unsplash, Google Drive, or your hosting.
                                </div>
                            </div>
                        </div>

                        <div className="flex justify-end gap-3 pt-4 border-t">
                            <button type="button" onClick={resetForm} className="px-6 py-3 font-bold text-slate-500 hover:bg-slate-100 rounded-xl transition">Cancel</button>
                            <button type="submit" disabled={submitting} className={`px-6 py-3 text-white font-bold rounded-xl shadow-lg flex items-center gap-2 ${editingId ? 'bg-blue-600 hover:bg-blue-700' : 'bg-purple-900 hover:bg-purple-800'}`}>
                    ...
                    {projects.map(p => (
                        <div key={p.id} className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden group hover:shadow-md transition-all">
                            <div className="h-48 bg-slate-100 relative">
                                <img src={p.type === 'video' ? p.thumbnail : p.after} alt={p.title} className="w-full h-full object-cover"/>
